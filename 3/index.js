...
const [aa, bb] = [a, b].map(s =>
  s.split(',').reduce(
    (cords, move) => {
      const direction = move[0];
      const distance = parseInt(move.slice(1));
      const position = cords[cords.length - 1];
      const newPosition = { ...position }; // dont mutate
      switch (direction) {
        case 'L':
...
const paths = [];
aa.forEach(({ x: x2, y: y2 }, aIndex) => {
  if (aIndex === 0) {
    return;
  }
  const { x: x1, y: y1 } = aa[aIndex - 1];
  bb.forEach(({ x: x4, y: y4 }, bIndex) => {
    if (bIndex === 0) {
      return;
    }
    const { x: x3, y: y3 } = bb[bIndex - 1];
    ...
    const u =
      ...
      (((x1 - x2) * (y1 - y3) - (y1 - y2) * (x1 - x3)) /
        ((x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4)));
    // intersection point has to be within BOTH segments
    if (t <= 1 && t >= 0 && u <= 1 && u >= 0) {
...
function search({ x, y }, children) {
  if (!children || children.length === 0) {
    return 0;
  } else {
    children.forEach(c => search(c));
  }
}

search({ x: 0, y: 0 }, [aa.slice(1), bb.slice(1)]);
